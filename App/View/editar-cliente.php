...
if(empty($clientId) || intval($clientId) == false){
    header('Location: ../listagem-clientes');
}

$clientObj = new ClienteController();
$data = $clientObj->getById((int)$clientId);
...
if (isset($_POST["edit"])) {
    ...
    $responseEdit = $clientObj->updateClient($data, (int)$clientId);
    if($responseEdit["status"] == false){
        errorMessage($responseEdit["message"]);
    }
    else {
        successMessage($responseEdit["message"]);
    }
}
?>
...
<body>
    ...
    <main>
        ...
        <h2>Preencha todos os dados corretamente para alterar os dados do cliente!</h2>
